 ...
 * @Version 1.0
 */
public class RestClientRegistrar implements ImportBeanDefinitionRegistrar, BeanFactoryAware, EnvironmentAware {
    ...
    public void registerBeanDefinitions(AnnotationMetadata importingClassMetadata,
        ...
        // attributes - > {clients:SayRestService}
        Map<String, Object> attributes = importingClassMetadata.getAnnotationAttributes(EnableRestClient.class.getName());
        Class<?>[] clientClasses = (Class<?>[]) attributes.get("clients");
        // 筛选出来所有的接口
        Stream.of(clientClasses)
                .filter(Class::isInterface) // 仅选择接口
                .filter(interfaceClass -> findAnnotation(interfaceClass, RestClient.class) != null) // 仅选择标注了@RestClient的接口
...
//                    String serviceName = restClient.value();
                    ...
                    // 方式一： FactoryBean注册BeanDefinition
//                    registerBeanDefinitionByFactoryBean(serviceName, proxy, restClientClass, registry);
                });
    }

    private void registerBeanDefinitionByFactoryBean(String serviceName, Object proxy, Class<?> restClientClass, BeanDefinitionRegistry registry) {
        String beanName = "RestClient." + serviceName;
